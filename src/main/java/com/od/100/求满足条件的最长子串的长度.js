...
var longSubstr = function (str) {
  let result = -1; // 存放结果，最大长度
  let pos = []; // 记录字母的位置

  pos.push(-1); // 第一个初始值，防止出现只有一个字母或者没有字母，无法进入循环的情况

  for (let i = 0; i < str.length; i++) {
    let ch = str.charAt(i);
    if ((ch >= "a" && ch <= "z") || (ch >= "A" && ch <= "Z")) {
      pos.push(i); // 存放所有字母出现的索引值
    }
  }

  pos.push(str.length); // 最后一个初始值，防止出现只有一个字母或者没有字母，无法进入循环的情况

  for (let i = 0; i < pos.length - 2; i++) {
    let diff = pos[i + 2] - pos[i]; // // 每隔一个字母差值，即可判断可以放多少个数字

    if (diff > 2) {
      result = Math.max(result, diff - 1); // // 出现更长的长度时才替换
    }
  }
