...
https://blog.csdn.net/weixin_44219664/article/details/123947690
...
function games() {
  let [M, N] = readline().trim().split(",").map(Number); // M 评委 N 选手
  ...
  players.sort((a, b) => {
    if (a.total < b.total) {
      ...
      let s2 = b.scores;
      for (let i = 10; i > 0; i--) {
        let ipl = checkCount(s1, i);
        let ith = checkCount(s2, i);
        if (ipl > ith) {
          return -1;
        } else if (ipl < ith) {
          return 1;
